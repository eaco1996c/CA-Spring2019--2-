...
bool Curve::findTimeInterval(unsigned int& nextPoint, float time)
{
	...
	if (nextPoint.inputTime!=NULL)
	{
		
		intervalTime = nextPoint.inputTime-time;
		return true;
	}
...
Point Curve::useHermiteCurve(const unsigned int nextPoint, const float time)
{
	...
	newPosition = (2.0f * time * time * time - 3.0f * time * time + 1.0f) * nextPoint
		+ (time * time * time - 2.0f * time * time + time) * m0
